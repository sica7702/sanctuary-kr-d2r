...
const PROFILE_FIELDS={
  ring:['itemtype','mode','realm'], boots:['itemtype','purpose','mode','realm'], belt:['itemtype','purpose','mode','realm'], crossbow:['itemtype','build','mode','realm'],
  amulet:['itemtype','char','build','mode','realm'], circlet:['itemtype','char','build','mode','realm'], gloves:['itemtype','char','build','mode','realm'], classarmor:['itemtype','char','build','mode','realm'], melee:['itemtype','char','build','mode','realm'], polearm:['itemtype','char','build','mode','realm'], wand:['itemtype','char','build','mode','realm'], caster:['itemtype','char','build','mode','realm'],
  bow:['itemtype','build','mode','realm'], jav:['itemtype','build','mode','realm'], claw:['itemtype','build','mode','realm'], orb:['itemtype','build','mode','realm']
};
const FIXED_CHAR={bow:'아마존',crossbow:'아마존',jav:'아마존',claw:'어쌔신',orb:'소서리스'};
const PROFILE_DEF={
 itemtype:['s','itemtype','아이템 유형',['레어','매직','캐스터 크래프트','블러드 크래프트','힛파워 크래프트','세이프티 크래프트']], char:['s','char','직업',Object.keys(BUILDS)], build:['s','build','빌드',BUILDS['아마존']],
...
gloves:[['st','skilltree_name','스킬트리 종류'],['n','skilltab','선택 스킬트리 +'],['n','ias','공격속도 %'],['n','str','힘'],['n','dex','민첩'],['n','ar','명중률(어레)'],['n','mana','마나'],['n','maxstam','최대 지구력'],['n','rep','생명력 회복(리플)'],['n','ll','생명력 훔침 %'],['n','ml','마나 훔침 %'],['n','fire','화염 저항'],['n','light','번개 저항'],['n','cold','냉기 저항'],['n','poison','독 저항'],['n','mf','매찬 %'],['n','gf','삥 / 적에게서 얻는 금화 % 증가']],
...
polearm:[['s','base','무기군',['아마존 스피어','스피어','폴암']],['n','offias','무기 외 IAS %'],['n','skillias','스킬 공속 보정 %'],['n','ed','피해 증가(ED) %'],['n','ias','공격속도 %'],['n','ar','추가 명중률'],['n','ll','생명력 훔침 %'],['n','ml','마나 훔침 %'],['n','sockets','소켓 수'],['c','fools','풀스(Fool\'s)'],['c','eth','에테리얼'],['c','repair','내구도 자동 회복'],['c','amp','앰플']],
claw:[['s','base','클러 베이스',BASE_CLAW],['n','offias','무기 외 IAS %'],['n','skillias','폭발적인 속도 등 스킬 IAS %'],['n','classskill','어쌔신 전체 기술 +'],['st','skilltree_name','스킬트리 종류'],['n','skilltab','선택 스킬트리 +'],['n','ias','공격속도 %'],['n','ed','피해 증가(ED) %'],['t','main_name','핵심 스태프모드 스킬명'],['n','main','번개 파수기 등 핵심 스태프모드 +'],['t','support_name','보조 스태프모드 스킬명'],['n','support','마인드블라스트/죽음파수기/블레이드실드 등 +'],['n','sockets','소켓 수'],['c','fools','풀스'],['c','eth','에테리얼'],['c','repair','내회']],
...
const MAX_CAPS={
 ...
 jav:{classskill:2,skilltab:2,ias:40,ed:450,ar:300,ll:9,ml:9}, polearm:{ed:450,ias:40,ar:300,ll:9,ml:9,sockets:2},
...
const isCraft=t=>String(t||itemType()).includes('크래프트');
const MAGIC_CAPS={ring:{mindmg:13,energy:20,mf:25},amulet:{skilltab:3,rep:15,mindmg:13,mf:35},circlet:{skilltab:3,rep:15,mindmg:13,maxdmg:12,mf:35},gloves:{skilltab:3},claw:{skilltab:3},orb:{skilltab:3},wand:{skilltab:3}};
function capOf(k){const t=itemType(),cc=((CRAFT_CAPS[t]||{})[active]||{})[k],mc=((MAGIC_CAPS[active]||{})[k]);if(t==='매직'&&mc!==undefined)return mc;return cc!==undefined?cc:(MAX_CAPS[active]||{})[k]}
function capHint(k){const t=itemType(),cap=capOf(k),note=((CAP_NOTES[active]||{})[k]);if(isCraft(t)&&((CRAFT_CAPS[t]||{})[active]||{})[k]!==undefined)return `표시 총합 최대 ${cap} · 고정옵+랜덤 접사 중첩 가능`;if(t==='매직')return cap!==undefined?`매직 검증 상한 ${cap}`:'';return note|| (cap!==undefined?`레어 최대 ${cap}`:'')}
...
function fieldHTML(f){
 if(f[0]==='n'){const cap=capOf(f[1]),note=capHint(f[1]);const hint=note?`<span class="maxhint">${esc(note)}</span>`:'';const mx=cap!==undefined?` max="${cap}"`:'';return `<div class="field"><label for="in_${f[1]}">${esc(f[2])}${hint}</label><input type="number" min="0"${mx} step="1" id="in_${f[1]}" placeholder="0"></div>`}
 if(f[0]==='t'){const staff=/^(main_name|support_name|skill_name)$/.test(f[1]);return `<div class="field"><label for="in_${f[1]}">${esc(f[2])}</label><input type="text" ${staff?'list="skrStaffSkillList"':''} id="in_${f[1]}" placeholder="게임 화면의 스킬명을 그대로 입력"></div>`;}
 if(f[0]==='c')return `<div class="field check"><label for="in_${f[1]}">${esc(f[2])}</label><input type="checkbox" id="in_${f[1]}"></div>`;
 if(f[0]==='st'){const trees=SKILL_TREES[profileChar()]||[];return `<div class="field"><label for="in_${f[1]}">${esc(f[2])}</label><select id="in_${f[1]}">${trees.map(x=>`<option>${esc(x)}</option>`).join('')}</select></div>`}
 return `<div class="field"><label for="in_${f[1]}">${esc(f[2])}</label><select id="in_${f[1]}">${f[3].map(x=>`<option>${esc(x)}</option>`).join('')}</select></div>`
}
function ensureStaffSkillDatalist(){if(document.getElementById('skrStaffSkillList'))return;const d=document.createElement('datalist');d.id='skrStaffSkillList';for(const sk of (window.SKR_BUILD_DEMAND?.staffmodSkills||[])){const o=document.createElement('option');o.value=sk.ko;o.label=sk.en||'';d.appendChild(o)}document.body.appendChild(d)}
function renderProfile(){
 const g=$('profileGrid'),fields=PROFILE_FIELDS[active]||[];g.innerHTML='';
 fields.forEach(k=>{let f=[...PROFILE_DEF[k]];if(k==='build'){const c=FIXED_CHAR[active]||'아마존';f[3]=BUILDS[c]}if(k==='itemtype'&&!['ring','amulet','gloves','boots','belt'].includes(active))f[3]=['레어','매직'];g.insertAdjacentHTML('beforeend',fieldHTML(f))});
...
function renderInputs(){
 const g=$('inputGrid');g.innerHTML='';
 g.insertAdjacentHTML('beforeend',`<div class="field"><label for="in_reqlevel">아이템 요구레벨 <span class="maxhint">LLD/MLD 판정용</span></label><input type="number" min="0" max="99" step="1" id="in_reqlevel" placeholder="0"></div>`);
 const t=itemType();
 if(isCraft(t))g.insertAdjacentHTML('beforeend',`<div class="field"><label for="in_craft_clvl">제작 캐릭터 레벨 <span class="maxhint">선택 입력 · craft ilvl 참고</span></label><input type="number" min="1" max="99" step="1" id="in_craft_clvl" placeholder="0"></div><div class="field"><label for="in_craft_ilvl">재료 아이템 레벨(ilvl) <span class="maxhint">선택 입력 · 도박/드랍 ilvl</span></label><input type="number" min="1" max="99" step="1" id="in_craft_ilvl" placeholder="0"></div>`);
 [...(INPUTS[active]||[]),...dynamicSkillFields()].forEach(f=>{if(f[1]==='mregen'&&['ring','amulet'].includes(active)&&t!=='캐스터 크래프트')return;g.insertAdjacentHTML('beforeend',fieldHTML(f))});
 craftExtraFields().forEach(f=>{if(!$('in_'+f[1]))g.insertAdjacentHTML('beforeend',fieldHTML(f))});
 const info=craftInfo();
 if(info)g.insertAdjacentHTML('beforeend',`<div class="profile-note" style="grid-column:1/-1"><b>크래프트 고정옵</b> · ${info}<br><span class="muted">입력칸의 최대치는 게임 화면에 표시될 수 있는 총합 기준입니다. 고정옵과 랜덤 접사가 같은 능력치를 올리면 합산될 수 있습니다.</span></div>`);
 if(t==='매직')g.insertAdjacentHTML('beforeend',`<div class="profile-note" style="grid-column:1/-1"><b>매직 전용 판정</b> · 최대 1접두/1접미 구조와 매직 전용 상한을 적용합니다. 시장 등급은 매직 득환 DB의 교차검증 유형과 별도로 대조합니다.</div>`);
 if(!AFFIX_SLOT[active])g.insertAdjacentHTML('beforeend',`<div class="profile-note" style="grid-column:1/-1"><b>검증 범위</b> · 이 부위는 옵션 수치와 기본 조건을 확인하지만 접두·접미 슬롯 수를 자동으로 확정하지 않습니다. 중요한 거래 전에는 게임 내 옵션과 접사 규칙을 별도로 대조하세요.</div>`);
 if(active==='classarmor')g.insertAdjacentHTML('beforeend',`<div class="profile-note" style="grid-column:1/-1"><b>전용 방어구 참고</b> · 하위 베이스 종류별 허용 옵션 검증은 아직 제공하지 않습니다.</div>`);
 $('resultBox').style.display='none';
 updateOptionSearch();
}
const OPTION_ALIASES={fcr:'패캐 시전속도',fhr:'패힛 타격회복',frw:'달려 이동속도',ias:'공속 공격속도',allres:'올레 모든저항',fire:'파레 화염저항',light:'라레 번개저항',cold:'콜레 냉기저항',poison:'포레 독저항',mf:'매찬 마법아이템발견',gf:'삥 골드 금화',ar:'어레 명중률',ed:'증뎀 피해증가',eddef:'방상 방어력증가',ll:'라흡 생명력흡수',ml:'마흡 마나흡수',rep:'리플 생명력회복',mpk:'마상 처치마나',sockets:'솟 소켓',str:'힘',dex:'민첩',life:'피 생명력',mana:'마나'};
function optionFilled(control){if(!control)return false;if(control.type==='checkbox')return control.checked;if(control.tagName==='SELECT')return control.selectedIndex>0;if(control.type==='number')return control.value!==''&&Number(control.value)!==0;return control.value.trim()!==''}
function updateOptionSearch(){
 const search=$('manualOptionSearch'),grid=$('inputGrid'),status=$('manualOptionStatus');if(!search||!grid||!status)return;
 const needle=search.value.toLowerCase().replace(/[\s-]/g,'');let matches=0,shown=0,filled=0;
 grid.querySelectorAll('.field').forEach(field=>{
  const control=field.querySelector('input,select'),hasValue=optionFilled(control),name=control?.id?.replace(/^in_/,'')||'';
  const words=((field.querySelector('label')?.textContent||'')+' '+name+' '+(OPTION_ALIASES[name]||'')).toLowerCase().replace(/[\s-]/g,'');
  const match=!needle||words.includes(needle);if(needle&&match)matches++;
  const show=match||hasValue;field.classList.toggle('manual-option-hidden',!show);field.classList.toggle('manual-option-filled',hasValue);
  if(show)shown++;if(hasValue)filled++;
 });
 status.textContent=needle?(matches?`${matches}개 검색 결과 · 입력한 옵션 ${filled}개는 계속 표시됩니다.`:`검색 결과가 없습니다. 다른 이름이나 약어로 찾아보세요.${filled?' 입력한 '+filled+'개 옵션은 유지됩니다.':''}`):`현재 부위 입력칸 ${shown}개 · 옵션명을 검색하면 해당 칸만 볼 수 있습니다.`;
}
function ensureManualSearchUI(){
 const partSearch=$('q'),partBox=partSearch?.closest('.searchbox');
 if(partBox&&!document.querySelector('.manual-section-label')){const label=document.createElement('label');label.className='manual-section-label';label.htmlFor='q';label.textContent='1. 감정할 부위 찾기';partBox.before(label);partSearch.placeholder='부위 검색: 반지, 목걸이, 장갑, 활, 클러…';}
 const head=document.querySelector('.appraise-head');if(head&&!head.querySelector('.manual-guide')){
  const title=head.querySelector('h3'),intro=head.querySelector('p');if(title)title.textContent='2. 옵션을 입력하고 감정하세요';
  if(intro){const full=intro.textContent;intro.textContent='사진의 옵션 수치만 입력하세요. 필요한 옵션이 안 보이면 아래 검색창에서 이름이나 약어로 찾을 수 있습니다.';const details=document.createElement('details');details.className='manual-guide';const summary=document.createElement('summary');summary.textContent='입력 방법과 검증 범위 자세히 보기';const body=document.createElement('p');body.textContent=full;details.append(summary,body);intro.after(details);}
 }
 const hero=document.querySelector('.hero-db');if(hero&&!hero.querySelector('.manual-skip')){const link=document.createElement('a');link.className='manual-skip';link.href='#manualOptionSearch';link.textContent='선택한 부위의 옵션 입력으로 이동 ↓';hero.append(link);}
 const grid=$('inputGrid');if(!grid||$('manualOptionSearch'))return;
 const bar=document.createElement('div');bar.className='manual-field-tools';const wrap=document.createElement('div');wrap.className='manual-search-wrap';const label=document.createElement('label');label.htmlFor='manualOptionSearch';label.textContent='옵션 찾기';const input=document.createElement('input');input.type='search';input.id='manualOptionSearch';input.placeholder='예: 패캐, 화염 저항, 소켓, 매찬';input.autocomplete='off';wrap.append(label,input);const clear=document.createElement('button');clear.type='button';clear.textContent='전체 옵션 보기';clear.addEventListener('click',()=>{input.value='';updateOptionSearch();input.focus()});bar.append(wrap,clear);
 const status=document.createElement('p');status.id='manualOptionStatus';status.className='manual-option-status';status.setAttribute('role','status');status.setAttribute('aria-live','polite');grid.before(bar,status);
 input.addEventListener('input',updateOptionSearch);input.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!=='ArrowDown')return;const first=grid.querySelector('.field:not(.manual-option-hidden) input,.field:not(.manual-option-hidden) select');if(first){e.preventDefault();first.focus()}});
 grid.addEventListener('input',updateOptionSearch);grid.addEventListener('change',updateOptionSearch);
}
function val(k){const e=$('in_'+k);if(!e)return 0;if(e.type==='checkbox')return e.checked?1:0;if(e.tagName==='SELECT')return e.value;if(e.type==='text')return e.value;return Number(e.value)||0}
...
function craftInfo(){const t=String(val('itemtype')||'레어');if(!isCraft(t))return '';return (CRAFT_FIXED[t]||{})[active]||'이 부위의 해당 크래프트 고정옵은 별도 레시피 검증이 필요합니다.'}
const AFFIX_SLOT={
...
function legality(warn){const t=itemType();
 ...
 if(t!=='매직'&&val('skilltab')>2&&['amulet','circlet','gloves','claw','orb','wand','caster'].includes(active))warn.push('+3 스킬탭은 매직 전용입니다. 스태프모드 +3과 혼동하지 마세요.');
 ...
 if(active==='circlet'&&t==='레어'&&val('maxdmg')>8)warn.push('레어 써클릿 최대피해는 8까지입니다. 9~12는 매직 전용입니다.');
...
function craftSpecialScore(id,st){const t=itemType();if(!isCraft(t))return 0;let b=0;
...
function craftIlvlInfo(){if(!isCraft())return '';const c=val('craft_clvl'),i=val('craft_ilvl');if(!c||!i)return '크래프트 ilvl: 제작/재료 레벨을 입력하면 참고값을 계산합니다.';const out=Math.floor(c/2)+Math.floor(i/2);return `크래프트 결과 ilvl 참고: ${out} = floor(${c}/2)+floor(${i}/2). 실제 접사 가능 여부는 베이스 qlvl/alvl까지 확인해야 하므로 여기서 확정하지 않습니다.`}
...
function expertNarrative(id,quality,st,warn){const good=[],bad=[],note=[];const v=k=>+val(k)||0, cls=v('classskill'), fcr=v('fcr'), frw=v('frw'), sock=v('sockets'), ar=v('ar'), str=v('str'), dex=v('dex'), life=v('life'), mana=v('mana'), all=v('allres'), ll=v('ll'), ml=v('ml');
 ...
 if(id==='circlet'){
  ...
  if(ar>=100||v('maxdmg')>=7)good.push('명중/피해 옵션이 있어 물리/PvP형 특화 가능성이 있습니다.');
...
const DEMAND_SLOT={ring:'반지',amulet:'목걸이',circlet:'써클릿',gloves:'장갑',boots:'부츠',belt:'벨트',melee:'무기',polearm:'무기',bow:'활',crossbow:'활',jav:'자벨린',claw:'클러',orb:'오브',wand:'완드',caster:'직업 전용',classarmor:'직업 전용'};
function demandClassSkillName(){const c=profileChar();return ({'아마존':'아마존 기술 레벨','소서리스':'원소술사 기술 레벨','네크로맨서':'강령술사 기술 레벨','팔라딘':'성기사 기술 레벨','야만용사':'야만용사 기술 레벨','드루이드':'드루이드 기술 레벨','어쌔신':'암살자 기술 레벨','악마술사':'악마술사 기술 레벨'})[c]||''}
function demandItems(){const a=[];const push=(n,k)=>{const x=+val(k)||0;if(x>0)a.push({name:n,value:x})};
 const common=[['시전 속도 증가','fcr'],['공격 속도 증가','ias'],['달리기/걷기 속도 증가','frw'],['타격 회복 속도 증가','fhr'],['힘','str'],['민첩','dex'],['마력','energy'],['생명력','life'],['마나','mana'],['마나 재생','mregen'],['명중률','ar'],['최소 피해','mindmg'],['최대 피해','maxdmg'],['피해 증가','ed'],['모든 저항','allres'],['화염 저항','fire'],['냉기 저항','cold'],['번개 저항','light'],['독 저항','poison'],['적중당 생명력 훔침','ll'],['적중당 마나 훔침','ml'],['마법 아이템 발견 확률','mf'],['적에게서 얻는 금화 증가','gf'],['생명력 회복','rep'],['소켓','sockets']];common.forEach(([n,k])=>push(n,k));
 const cs=demandClassSkillName();if(cs)push(cs,'classskill');
 const tree=String(val('skilltree_name')||'');if(tree.includes('투창'))push('투창과 창 기술 레벨','skilltab');if(tree.includes('활과'))push('활과 쇠뇌 기술 레벨','skilltab');if(tree.includes('무술'))push('무술 기술 레벨','skilltab');
 if(+val('main')>0||+val('staff1')>0)a.push({name:'개별 기술 레벨 (스태프모드)',value:Math.max(+val('main')||0,+val('staff1')||0)});
 const addStaff=(nameKey,valueKey)=>{const txt=String(val(nameKey)||'').trim(),v=+val(valueKey)||0;if(!txt||!v)return;const sk=window.SKR_BUILD_DEMAND?.resolveStaffmodSkill?.(txt,DEMAND_SLOT[active]||'');if(sk)a.push({name:sk.key,value:v});};
 if(active==='classarmor'){addStaff('main_name','staff1');addStaff('support_name','staff2')}else{addStaff('main_name','main');addStaff('support_name','support')}
 if(+val('eth'))a.push({name:'에테리얼',value:1});if(+val('repair'))a.push({name:'내구도 자동 회복',value:1});if(+val('replenish'))a.push({name:'수량 자동 회복',value:1});if(+val('amp'))a.push({name:'피해 증폭 발동',value:5});
 return a;}
function inferBuildDemand(){const slot=DEMAND_SLOT[active];if(!slot||!window.SKR_BUILD_DEMAND)return {score:0,label:'D',best:null,top:[]};return window.SKR_BUILD_DEMAND.evaluate(slot,demandItems())}


function runtimeRuleContext(){
 const values={reqlevel:val('reqlevel')};
 for(const f of [...(INPUTS[active]||[]),...dynamicSkillFields(),...craftExtraFields()]){
   if(!f||!f[1])continue; values[f[1]]=val(f[1]);
 }
 return {
   slot:active,
   item_type:itemType(),
   profile:{
     char:profileChar(),build:profileBuild(),purpose:val('purpose'),
     mode:val('mode'),realm:val('realm')
   },
   values
 };
}
...
if(['claw','orb','wand','caster','classarmor'].includes(id)){const names=[String(val('main_name')||'').trim(),String(val('support_name')||'').trim()].filter(Boolean);const exact=names.map(n=>window.SKR_BUILD_DEMAND?.resolveStaffmodSkill?.(n,DEMAND_SLOT[id]||'')).filter(Boolean);if(exact.length)st.push('정확 스태프모드 판독: '+exact.map(x=>`${x.ko}(${x.tier>=5?'핵심':x.tier===4?'상급':'유틸'})`).join(' · '));}
const chn=String(val('charge_skill_name')||'').trim(), chlv=val('charge_skill_level'), chc=val('charge_count');if(chn){st.push(`충전 기술: ${chn}${chlv?` Lv${chlv}`:''}${chc?` · 최대 ${chc}회`:''}`);}const pnm=String(val('proc_skill_name')||'').trim(), plv=val('proc_skill_level'), pch=val('proc_chance'), ptr=String(val('proc_trigger')||'');if(pnm){st.push(`발동 기술: ${pnm}${plv?` Lv${plv}`:''}${pch?` · ${pch}%`:''}${ptr&&ptr!=='없음/선택'?` · ${ptr}`:''}`);} const craftAdj=craftSpecialScore(id,st);const demand=inferBuildDemand();const runtime=window.SKR_RUNTIME_RULES?.evaluate(runtimeRuleContext())||{scoreDelta:0,matched:[],strengths:[],warnings:[],online:false,revision:null};if(runtime.strengths?.length)st.push(...runtime.strengths.map(x=>'운영 DB · '+x));if(runtime.warnings?.length)warn.push(...runtime.warnings.map(x=>'운영 DB · '+x));const quality=Math.max(0,Math.min(Math.round(s+craftAdj+(runtime.scoreDelta||0)),100)),adj=profileAdjust(id,st),fit=buildFitLabel(adj),mc=marketConfidence(warn,quality),g=itemType()==='매직'?['검토','매직 아이템은 1접두·1접미 생성 규칙과 입력 롤을 검증했습니다. 확정 등급은 검증 유형과 완전히 일치할 때만 이미지 감정에서 제공합니다.']:grade(quality);$('resultBox').style.display='block';$('resultGrade').textContent=g[0];$('resultScore').textContent='아이템 품질 '+quality+' / 100 · 선택빌드 '+fit+' · 자동 빌드수요 '+(demand.score||0)+'% · 시장 신뢰도 '+mc[0]+(runtime.matched?.length?' · 운영DB '+(runtime.scoreDelta>=0?'+':'')+runtime.scoreDelta+' ('+runtime.matched.length+'규칙)':'');$('resultSummary').innerHTML=`${esc(g[1])}<br><b>생성 검증:</b> ${warn.length?'재확인 항목 있음':'1차 통과'} · <b>시장 판단:</b> ${esc(mc[1])}<br>${profileParts().length?'선택: <b>'+profileParts().map(esc).join(' · ')+'</b><br>':''}부위: <b>${esc((DB.categories.find(x=>x.id===id)||{}).title||id)}</b>${baseInfo()?'<br>'+baseInfo():''}${frameInfo()?'<br>'+frameInfo():''}<br>${esc(reqLevelCheck([]))}${affixStructure([])?'<br>'+esc(affixStructure([])):''}${craftInfo()?'<br>'+esc('크래프트 고정옵: '+craftInfo()):''}${isCraft()?'<br>'+esc(craftIlvlInfo()):''}${rollBreakdown()?'<br><b>주요 롤:</b> '+esc(rollBreakdown()):''}`;const ex=expertNarrative(id,quality,st,warn);$('resultStrength').innerHTML=[...st,...ex.good].map(x=>'◆ '+esc(x)).join('<br>');$('resultWeak').innerHTML=ex.bad.map(x=>'◆ '+esc(x)).join('<br>');$('resultExpert').innerHTML=professionalVerdict(id,quality,ex)+(ex.note?'<br><br><span style="color:#8fa0b5">'+esc(ex.note)+'</span>':'');const dtop=(demand.top||[]).filter(x=>x.score>=35).slice(0,3);$('resultBuildDemand').innerHTML=dtop.length?dtop.map((x,i)=>{const rs=(x.reasons||[]).slice(0,2),gp=(x.missing||[]).slice(0,3);return `${i===0?'◆':'◇'} <b>${esc(x.name)}</b> · ${x.score}% (${esc(window.SKR_BUILD_DEMAND.label(x.score))})<br><span style="color:#7f8994">${esc(x.use)}${x.notes?' · '+esc(x.notes):''}</span>${rs.length?`<br><span style="color:#9fd6b4">맞는 이유: ${rs.map(esc).join(' · ')}</span>`:''}${gp.length?`<br><span style="color:#c7a6a0">상위형 결손: ${gp.map(esc).join(' · ')}</span>`:''}`}).join('<br><br>'):'현재 옵션 조합에서 특정 빌드 프리미엄은 뚜렷하지 않습니다.';$('resultWarn').innerHTML=(warn.length?warn.map(x=>'주의 · '+esc(x)).join('<br>'):'주의 · 자동 감정은 1차 선별용입니다.')+'<br>직업/빌드 가중치는 생성 가능 여부를 바꾸지 않습니다. 최종 가격은 베이스·요구레벨·PvM/PvP·래더/스탠 실거래를 교차검증하세요.'}
function assessWithReviewValue(){
 window.SKR_AI_APPRAISAL?.cancel();
 assess();
 $('reviewValueResult')?.remove();
 const learnedContext=runtimeRuleContext();
 if(String(learnedContext.item_type).includes('크래프트'))learnedContext.item_type='크래프트';
 const aiWarnings=[];legality(aiWarnings);
 const originalContext=JSON.stringify(runtimeRuleContext());
 window.SKR_AI_APPRAISAL?.apply({context:learnedContext,heading:$('resultGrade'),anchor:$('resultSummary'),eligible:!aiWarnings.length,
  isCurrent:()=>$('resultBox').style.display!=='none'&&JSON.stringify(runtimeRuleContext())===originalContext});
 const reviewValue=window.SKR_RUNTIME_RULES?.evaluateValue?.(learnedContext);
 if(reviewValue?.status!=='learned')return;
 const note=document.createElement('p');note.id='reviewValueResult';note.className='muted';
 note.textContent='검수 학습 참고: '+reviewValue.label+' · 근거 '+reviewValue.sample_count+'건';
 $('resultSummary').after(note);
}
window.addEventListener('skr-review-value-model-ready',()=>{if($('resultBox')?.style.display==='block')assessWithReviewValue();});
function resetInputs(){for(const k of ['reqlevel','craft_clvl','craft_ilvl']){const e=$('in_'+k);if(e)e.value=''};[...(INPUTS[active]||[]),...dynamicSkillFields(),...craftExtraFields()].forEach(f=>{const e=$('in_'+f[1]);if(!e)return;if(e.type==='checkbox')e.checked=false;else if(e.tagName==='SELECT')e.selectedIndex=0;else e.value=''});$('resultBox').style.display='none';updateOptionSearch()}
function renderTabs(filter=''){const t=$('tabs');t.innerHTML='';const f=filter.trim().toLowerCase();let shown=DB.categories.filter(c=>!f||JSON.stringify(c).toLowerCase().includes(f));if(!shown.length){t.innerHTML='<span class="muted">검색 결과 없음 · 현재 입력은 유지됩니다.</span>';return}shown.forEach(c=>t.insertAdjacentHTML('beforeend',`<button class="tab ${c.id===active?'active':''}" data-id="${c.id}" aria-pressed="${c.id===active}">${c.short}</button>`));if(!shown.some(c=>c.id===active))t.insertAdjacentHTML('beforeend','<span class="muted">원하는 부위를 선택하세요. 입력 중인 값은 검색만으로 지워지지 않습니다.</span>');t.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{active=b.dataset.id;if($('manualOptionSearch'))$('manualOptionSearch').value='';renderTabs($('q').value);render();renderProfile();renderInputs()})}
...
ensureStaffSkillDatalist();ensureManualSearchUI();$('audit').innerHTML=DB.audit.map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(x.fact)}</td><td>${esc(x.source)}</td></tr>`).join('');$('q').addEventListener('input',e=>renderTabs(e.target.value));$('clear').onclick=()=>{$('q').value='';active=DB.categories[0].id;if($('manualOptionSearch'))$('manualOptionSearch').value='';renderTabs();render();renderProfile();renderInputs()};$('runAppraise').onclick=assessWithReviewValue;$('resetAppraise').onclick=resetInputs;renderTabs();render();renderProfile();renderInputs();
